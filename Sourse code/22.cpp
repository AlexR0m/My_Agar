...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		for (int i = 0; i<ASTEROID_COUNT; i++)
		{
			...
			hOldPen = (HPEN)SelectObject(backbuffDC, hPen);
			Ellipse(backbuffDC, asteroids[i].X - asteroids[i].r, asteroids[i].Y - asteroids[i].r,
				asteroids[i].X + asteroids[i].r, asteroids[i].Y + asteroids[i].r);
			SelectObject(backbuffDC, hOldPen);
			DeleteObject(hPen);
			hPen = CreatePen(PS_SOLID, asteroids[i].r, RGB(0, 0, 255));
			hOldPen = (HPEN)SelectObject(backbuffDC, hPen);
			Ellipse(backbuffDC, asteroids[i].X - (asteroids[i].r / 2), asteroids[i].Y - (asteroids[i].r / 2),
				asteroids[i].X + (asteroids[i].r / 2), asteroids[i].Y + (asteroids[i].r / 2));
			SelectObject(backbuffDC, hOldPen);
			DeleteObject(hPen);
		}
		...
		SelectObject(backbuffDC, hOldPen);
		DeleteObject(hPen);
		hPen = CreatePen(PS_SOLID, ship.health, RGB(0, 255, 0));
		hOldPen = (HPEN)SelectObject(backbuffDC, hPen);
		Ellipse(backbuffDC, ship.X - (ship.health / 2), ship.Y - (ship.health / 2), ship.X + (ship.health / 2), ship.Y + (ship.health / 2));
		SelectObject(backbuffDC, hOldPen);
		DeleteObject(hPen);
		...
		if (GameOver == 1){
			...
			TextOut(backbuffDC, 365, 275, s, wcslen(s));
			///Conclusion inscriptions "Press R to restart the game!".
			wsprintf(s, _T("Press R to restart the game!"), ship.health - 20);
			TextOut(backbuffDC, 300, 300, s, wcslen(s));
		}
		/// The conclusion points during the game
		wsprintf(s, _T("score %d"), ship.health - 20);
		TextOut(backbuffDC, 20, 630, s, wcslen(s));
		///Ver game
		wsprintf(s, _T("ver 1.0 (Yakudza)"));
		TextOut(backbuffDC, 650, 630, s, wcslen(s));
		///Creator
		wsprintf(s, _T("Creator Alexander Romanov."));
		TextOut(backbuffDC, 20, 20, s, wcslen(s));
		...
		if (pause)
		{
			...
			TextOut(backbuffDC, 350, 250, s, wcslen(s));
			///reference
			wsprintf(s, _T("Press P to continue playing!"));
			TextOut(backbuffDC, 300, 630, s, wcslen(s));
		}
		else
		{
			///reference
			wsprintf(s, _T("Press P to pause!"));
			TextOut(backbuffDC, 325, 630, s, wcslen(s));
		}
